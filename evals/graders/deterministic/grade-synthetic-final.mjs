 ...
 *
 *   node evals/graders/deterministic/build-synthetic-final.mjs
 *   node evals/graders/deterministic/grade-synthetic-final.mjs
 */
...
const SYN_ROOT = path.join(EVALS_ROOT, "runs/synthetic-final");

function main() {
	if (!fs.existsSync(path.join(SYN_ROOT, "index.json"))) {
		console.error(
			"No synthetic-final index. Run: node evals/graders/deterministic/build-synthetic-final.mjs",
		);
		process.exit(2);
	}
	...
	try {
		...
		console.error(`invalid synthetic-final index.json: ${err.message}`);
		process.exit(2);
	}
	...
	fs.writeFileSync(path.join(reportsDir, "final-synthetic-latest.md"), md);
