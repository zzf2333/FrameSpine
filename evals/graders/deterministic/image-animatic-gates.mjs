...
function hasStoryConfirmation(events) {
  return events.some((ev) => {
    ...
    if (t === 'stage_advance' && String(ev.to || '').includes('image-animatic')) {
...
export function gradeImageAnimaticWorkspace(workspace, caze, options = {}) {
  ...
  const ep = {
    ...
    composition:
      path.join(workspace, 'video/composition.html'),
  ...
  if (wants('surface')) {
    ...
    if (!preview) {
      ...
      if (STORYBOARD_SURFACES.has(String(preview.surface || '').toLowerCase())) {
        ok = false;
        push(
          'P0',
          'surface',
  ...
  if (wants('low_cost_media')) {
    ...
    if (composition?.final_polish) {
      ok = false;
      push('P0', 'low_cost_media', 'final polish claimed during Image Animatic');
    }
  ...
  if (wants('no_formal_tts')) {
    ...
    if (preview?.formal_tts) {
      ok = false;
      push('P0', 'no_formal_tts', 'formal_tts claimed true in preview-manifest');
    }
  ...
  if (wants('stage_boundary')) {
    let ok = true;
    if (!hasStageStop(events, 'image-animatic') && !hasStageStop(events, 'animatic')) {
