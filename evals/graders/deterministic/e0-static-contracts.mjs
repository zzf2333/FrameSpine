...
const MUST_CONTAIN = [
	...
	},
	{
		...
		patterns: [/每张图片需要 id、prompt 和 output/],
	},
];
...
const MUST_NOT_CONTAIN = [
	...
	},
	{
		...
		patterns: [
			{
				re: /audience_discovery\s*:/,
				label: "new metadata key audience_discovery",
			},
			{ re: /visual_event\s*:/, label: "new metadata key visual_event" },
			{
				re: /placeholder_risk\s*:/,
				label: "new metadata key placeholder_risk",
			},
			{
				re: /status\s*→\s*(sketch|reference|draft|evidence)/,
				label: "non-official status enum",
			},
		],
	},
];
...
function isProhibitionContext(text, index) {
	...
	return /不(?:要|能|得|会|应|可|把它做成|做成|建立|启用|引入)|禁止|避免|不是/.test(
		sentence,
	);
}
...
function checkImagePromptsTemplate() {
	...
	if (!Array.isArray(data)) {
		findings.push({
			level: "P0",
			check: "image-prompts-json",
			message: "template must be an array",
		});
		return findings;
	}
...
function main() {
	...
	for (const rel of ["SKILL.md", "references/core/episode-production.md"]) {
		...
		const text = read(rel);
		if (
			/evals\/cases\//.test(text) ||
			/Visual-Only Comprehension Rate/.test(text)
		) {
			findings.push({
				level: "P0",
