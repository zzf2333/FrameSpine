...
function isStudioRouteOpened(event, studioUrl, route) {
  const type = eventType(event);
  return (
    (type === 'studio_route_opened' || type === 'studio_opened' || Boolean(event.studio_route_opened)) &&
...
function isStageHandoff(event, studioUrl, route) {
  const type = eventType(event);
  return (
    (type === 'stage_handoff' || type === 'handoff' || Boolean(event.user_handoff)) &&
    matchesUrl(event, studioUrl) &&
    matchesRoute(event, route)
  );
}
...
function sourceWasMisrepresented(events) {
  return events.some((event) => {
    const type = eventType(event);
    const surface = String(event.surface || event.review_surface || '').toLowerCase();
    const target = String(event.url || event.route || event.path || '');
    return (
      (Boolean(event.user_preview) || Boolean(event.user_handoff) || type === 'stage_handoff') &&
...
export function verifyCompositionUserSurface(preview, events = []) {
  ...
  if (!STUDIO_REVIEW_SURFACES.has(reviewSurface)) {
    failures.push('review_surface is not an official Studio Composition surface');
  }
  if (!isHttpUrl(studioUrl)) {
    failures.push('missing HTTP(S) studio_url');
  }
  if (!route || isImplementationPath(route)) {
    failures.push('composition_route is missing or points to an implementation file');
  }
  if (sourceWasMisrepresented(events)) {
    failures.push('snapshot, source file, or implementation path was marked as user preview');
  }
  ...
  if (opened === -1) failures.push('Studio route was not opened after Studio start');
  if (played === -1) failures.push('Composition was not fully played after route open');
  if (handedOff === -1) failures.push('user handoff did not occur after full playback');

  return {
    ok: failures.length === 0,
    failures,
    evidence: { studio_url: studioUrl || null, composition_route: route || null, started, opened, played, handed_off: handedOff },
  };
}
