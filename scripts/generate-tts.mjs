...
async function main() {
  ...
  const input = path.resolve(project, args.input || "narration.txt");
  const output = path.resolve(project, args.output || "assets/audio/narration.wav");
  const { env } = await loadSeriesEnv(project);
  need(env, ["VOLCENGINE_TTS_APP_ID", "VOLCENGINE_TTS_ACCESS_TOKEN", "VOLCENGINE_TTS_VOICE_TYPE"]);

  const units = paragraphs(await readFile(input, "utf8"));
  if (!units.length) throw new Error("narration.txt 里没有旁白");

  const endpoint = env.VOLCENGINE_TTS_ENDPOINT || "https://openspeech.bytedance.com/api/v1/tts";
  ...
  for (let index = 0; index < units.length; index += 1) {
    ...
    const file = path.join(unitDir, `${String(index + 1).padStart(2, "0")}.wav`);
  ...
  await writeFile(output.replace(/\.[^.]+$/, ".json"), `${JSON.stringify({
    provider: "volcengine",
    voice_type: env.VOLCENGINE_TTS_VOICE_TYPE,
