...
“避免 AI 味”不是一句反向提示。对 Documentary Reality、Educational Narrative 中的典型处境、证据和人物过程素材，让模型少自由发挥：根据镜头任务写少量、具体且可见的真实线索。
...
Cinematic Storytelling、Conceptual Fantasy、metaphor 或 imagined world 不必伪装成普通生活摄影；它们应改为写 **Material Coherence** 与 **World-Rule Consistency**：材质、空间、光线、比例、因果和世界规则在内部可信，并符合本期 Narrative Contract。无论何种模式，畸形手、空间矛盾、人物身份漂移、物件数量/形态失控或关键证据不可读仍是失败。精确文字、数字和界面信息继续在 HyperFrames 中叠加，不强求模型生成。
...
| 人物处境 | 人物的具体选择、压力或代价，以及与对象和环境的关系 | 孤立肖像、泛泛情绪或面对镜头表演困扰。 |
| 证据 | 证据来源、一个关键事实、可读区域、功能化普通光线 | 强景深、戏剧布光和让模型生成精确文字。 |
| 行为过程 | 动作中间状态、手与物件关系、前后步骤连续 | 只展示最终成品。 |
| 情绪承载 | 人与空间的关系、身体状态、停顿、自然观察距离 | 夸张表情和头抱手的陈词滥调。 |
| 对比 | 固定构图与不变变量，只改变需要被理解的关键变量 | 同时改变人物、空间、时间和色调，令因果失焦。 |
| 机制 | 可见因果、路径、层次、可分离对象与逐层发现 | 无根的图标、装饰性信息图或一屏解释一切。 |
| 后果 | 具体结果、数量累积、空间或状态改变 | 抽象地“显得很惨”却没有可见代价。 |
| 过渡 | 携带前一镜的物件、动作、方向或问题进入下一空间 | 无关空镜或为了转场而另起一张风格图。 |
...
3. 它继承哪些人物、空间与物件锚点？
4. 它相对前后素材新增什么镜头任务？
5. 对写实素材：哪些使用痕迹让它不像图库或广告摆拍？对概念 / 幻想素材：哪些材质与世界规则使它内部可信？
