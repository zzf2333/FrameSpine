...
> **最高原则：Narrative Contract 决定观众进入什么叙事体验；Visual Medium 决定该体验通过什么视觉材料呈现；二者互相约束，但互不选择。** 纪录现实可以使用漫画，电影叙事可以使用剪纸，概念幻想也可以使用写实摄影。不要把 Narrative Mode、Reality Level 或 Story License 当作媒介的允许值列表。

确定：

- 主媒介、允许的辅助媒介及每种媒介的叙事角色；
- 跨媒介怎样保持统一、何时允许发生有意转换；
- 人物、环境、物件、证据、机制图和视觉隐喻的长期分工；
- 图片主要承担人物处境、环境关系、机制解释或视觉隐喻中的哪些角色；
- 构图与观察方式、字幕安全区，以及字幕可读性从文字层级、阴影 / 描边到局部背景的优先顺序；
- 文字、数字、界面与证据的信息归属：哪些可自然存在于生成媒体，哪些必须精确并由真实素材或 HyperFrames 实现；
- 静态媒体进入 Composition 后的默认时间行为、有意 Hold 的条件，以及 Final 替换媒体后必须保留或重建的时间行为；
- 允许的图片结构与明确避免的无意媒介混杂；
...
- 竖屏、字幕和平台 UI 安全，以及 Caption Surface Test 是否避免全宽 / 双重字幕底板；
- Critical Information Test 是否让必要文字、数字、来源、证据和界面在生成媒体、真实素材与 HyperFrames 间有明确分工；
- `Motion Affordances` 在 HyperFrames 中是否成立；Motion Execution Test 是否在完整 Scene 的开始、中段、结束前验证运动或有意 Hold 与 Handoff；
- Provider 稳定性及常见失败模式；
- 声音、转场与可复用组件；
