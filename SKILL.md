...
在已确认的当前阶段方向、`DESIGN.md`、`Motion Language`、素材 Brief 与用户明确要求范围内，Agent 正常执行并自审普通制作动作；用户审的是方向和阶段成果，不是每个遮罩、标记或 Tween。

cover / crop、必要遮罩、已计划 Reveal、字幕安全处理、证据标记、当前 `Motion Language` 内的运动，以及 Scene / Layer 生命周期修复，不需要逐项重新请求用户确认，只要它们服务既有观看任务且不改变已确认方向。

仅当新增处理会实质改变 Visual Medium / 媒介配方、叙事意义、Attention Path、主要视觉事件、长期系列基线、可复用组件 / 后续多期继承处理，或明显提高返工成本的方向变化时，才作为候选 Studio Preview 单独展示其观看问题、位置与代价；展示后停止，等待用户在后续消息决定是否合并。不能因为“更有质感”、技术可运行或 Agent 偏好而直接加入实质方向变化。

此规则不建立批准记录，也不替代四个成熟阶段展示真实 Studio Preview 后停止、等待后续用户消息的边界。
...
同一张图片可以通过裁切、拉远、局部揭示、层级变化和图形连接承担多个节拍。不要默认一句旁白配一张图，也不要让所有画面重复同一种运动。

把占位画面升级为 Image Animatic，再交给用户观看。

## 6. 生成正式声音并制作 Timed Animatic

在调用正式 TTS、转写或写入 `captions.json` **之前**，先向用户说明：这一轮将把确认的图片结构置入正式声音、真实时长与 `captions.json`，并重新导演停留、节奏和声画同步；用户随后只需判断声音方向、字幕断句、声画同步、快慢与阅读时间。不要在这些工作完成后补报，也不会在这一轮批量最终生图、Final 精修或导出。
...
在批量最终图片生产 **之前**，先向用户说明：这一轮将生成并替换已确认方向的最终资产，统一图片与声音风格，并精修已通过的镜头语言；用户随后需判断整体统一性、镜头感、字幕舒适度、音乐/声音与发布准备度。不要在图片已经生成、替换或精修后补报；不会在 Final Preview 前导出。
...
逐素材完成 Prompt Audit 后，再完成 Image Set Audit，检查整组角色分布、观察方式、主体状态、媒介表现、功能性细节与观看任务不会同质化；这两项是 Agent 的人工导演检查，不是脚本评分或批准状态。`prompt` 留空会令脚本安全报错，绝不在实际字段放教学说明。先生成视觉世界锚点、第一帧、长期主体、核心重构、机制和结尾回收等高风险素材；在同一真实竖屏 HyperFrames Composition 中应用 cover/crop、字幕安全区、当前媒介允许的 Motion Affordances 与 Handoff 后再判断。用户不满意时，根据问题修改：视觉任务、锚点/参考、构图、连续性、Prompt 或图片方案，而不是反复使用同一个 Prompt 抽卡。不要在这一原则上建设自动 Prompt 编译器。
