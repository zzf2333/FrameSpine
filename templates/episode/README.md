...
- `STORYBOARD.md`：图片节拍、字幕与图片如何配合、素材策略和 HyperFrames 导演设计；它通过字幕 ID 引用 `captions.json`，不维护第二份字幕时间轴。Story Flow 时，全部主要 Beat 必须进一步注册为带可渲染 `src` 的官方 HyperFrames Storyboard items，Markdown 本身不是用户预览。
...
- `assets/`：声音、图片和参考素材。
- `video/`：同一个 HyperFrames 项目；先承载 Story Flow 的官方 Storyboard Cards，再从 Image Animatic 开始建立完整时间播放并持续演进到 Final。
